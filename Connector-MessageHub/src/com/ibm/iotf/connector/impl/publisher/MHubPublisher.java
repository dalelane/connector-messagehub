...
public class MHubPublisher implements EventRepublisher {
	...
	public MHubPublisher(MessageHubEnvironment env, Properties producerProperties) throws IOException {
		this.env = env;
		this.producer = createProducer(producerProperties);
		logger.log(Level.INFO, "Kafka producer created.");
	}

	private KafkaProducer<byte[], byte[]> createProducer(Properties producerProperties) throws IOException {
